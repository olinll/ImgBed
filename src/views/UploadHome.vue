<template>
    ...
    <div class="upload-home">
        ...
        <div class="header">
            <h1 class="title"><span class="main-title">{{ ownerName }}</span></h1>
        </div>
        <UploadForm 
...
export default {
    ...
    computed: {
        ...
        ownerName() {
            return this.userConfig?.ownerName || 'ImgBed'
        },
        dialogWidth() {
    ...
    mounted() {
        ...
        // 读取用户选择的上传渠道
        this.uploadChannel = this.storeUploadChannel || this.userConfig?.defaultUploadChannel || 's3'
        // 用户定义的失败自动切换
    ...
    methods: {
        ...
        handleMobileMenuCommand(command) {
            if (command === 'toggleTheme') {
                ...
                window.open('https://github.com/olinll/ImgBed', '_blank')
