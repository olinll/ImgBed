            ...
            <div class="upload-list-container" :class="{'upload-list-busy': fileList.length}">
                <el-scrollbar @scroll="handleScroll" ref="scrollContainer">
                    ...
                    <UploadFileItem
                        ...
                        @copy="handleCopy"
                        @remove="handleRemove"
                        @preview-error="(f) => f.url = '/static/media/404.png'"
                    />
                </el-scrollbar>
            </div>
        </el-card>
...
methods: {
    ...
    beforeUpload(file) {
        return new Promise(async (resolve, reject) => {
            ...
            const isLtLim = processedFile.size / 1024 / 1024 <= 1024
            ...
            if (needCustomCompress) {
                ...
                imageConversion.compressAccurately(processedFile, 1024 * this.compressQuality).then((res) => {
                    ...
                    //服务端压缩功能已移除
                    const needServerCompress = false

                    if (myUploadCount === 0) {
                        pushFileToQueue(newFile)
                    } else {
                        setTimeout(() => {
                            pushFileToQueue(newFile)
                            this.uploadCount--
